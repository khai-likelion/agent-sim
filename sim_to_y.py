...
class ComparisonReportGenerator:
    ...
    def _resolve_generation_label(generation: str, special_condition: str) -> str:
        ...
        if not isinstance(generation, str) or not generation.startswith("혼합"):
            return generation
        sc = "" if pd.isna(special_condition) else str(special_condition).strip()
        if not sc:
            return generation
    ...
    def _savefig(self, name: str, fmt: str = "svg") -> Path:
        """차트 저장. SVG가 기본값(벡터·소용량). 워드클라우드 등 래스터는 fmt='png' 지정."""
        path = self.figures_dir / f"{name}.{fmt}"
    ...
    def _get_store_tags(self, ratings_json: Dict, store_name: str) -> Dict[str, int]:
        """store_ratings.json에서 특정 매장의 태그 카운트 추출."""
        for store in ratings_json.get("stores", []):
            if store.get("store_name") == store_name:
                return {
                    "맛":    store.get("taste_count",      0),
                    "가성비": store.get("value_count",      0),
                    "분위기": store.get("atmosphere_count", 0),
                    "서비스": store.get("service_count",    0),
                }
        return {"맛": 0, "가성비": 0, "분위기": 0, "서비스": 0}
    ...
    def _plot_wordcloud(self, t1: pd.DataFrame, t2: pd.DataFrame):
        """LLM 기반 해시태그 키워드 추출 및 시각화."""
        import random

        def collect_texts(df: pd.DataFrame) -> str:
            parts: List[str] = []
            if "comment" in df.columns:
                parts.extend(df["comment"].dropna().astype(str).str.strip().tolist())
            if "reason" in df.columns:
                parts.extend(df["reason"].dropna().astype(str).str.strip().tolist())
            return "\n".join(p for p in parts if p and p != "nan")

        def extract_hashtags(text: str, label: str) -> List[str]:
            if not text.strip():
                return ["#데이터없음"]
            text_preview: str = text[0:3000]  # type: ignore[no-matching-overload]
            prompt = (
                f"다음은 '{label}' 매장의 방문자 리뷰입니다:\n\n{text_preview}\n\n"
                "이 방문자 리뷰들을 분석해서, 매장의 특징, 고객 경험, 개선점 등을 가장 잘 나타내는 "
                "직관적인 핵심 해시태그 10~15개를 쉼표로 구분해서 뽑아줘. "
                "예: #분위기맛집, #웨이팅길다, #가성비좋음\n"
                "해시태그만 쉼표로 구분해서 응답해. 다른 설명은 불필요해."
            )
            try:
                llm = LLMClient()
                response = llm.generate_sync(prompt)
                raw_tags: List[str] = [t.strip() for t in re.split(r"[,\n]", response) if t.strip()]
                clean_tags: List[str] = [t if t.startswith("#") else f"#{t}" for t in raw_tags if t and t != "#"]
                limited: List[str] = clean_tags[0:15]
                return limited if limited else ["#키워드추출실패"]
            except Exception as e:
                print(f"  [경고] LLM 해시태그 추출 실패: {e}")
                return ["#LLM오류", "#데이터확인필요"]

        def draw_hashtag_cloud(ax, tags: List[str], title: str, palette: List[str]):
            ax.set_xlim(0, 10)
            ax.set_ylim(0, 10)
            ax.axis("off")
            ...
            for i, tag in enumerate(tags):
                row = i // cols
                col = i % cols
                x = 0.6 + col * x_step + rng.uniform(-0.15, 0.15)
                y = 9.2 - row * y_step - rng.uniform(0.1, 0.4)
                fontsize = rng.randint(11, 19)
                color = palette[i % len(palette)]

                kwargs = dict(
                    fontsize=fontsize,
                    color=color,
                    alpha=rng.uniform(0.80, 1.0),
                    fontweight="bold",
                    ha="left",
                    va="center",
                    bbox=dict(
                ...
                ax.text(x, y, tag, **kwargs)

        texts1 = collect_texts(t1)
        texts2 = collect_texts(t2)

        print("  [1-WC] LLM 해시태그 추출 중 (Sim 1)...")
        tags1 = extract_hashtags(texts1, f"{self.target_store} 전략 전")
        print("  [1-WC] LLM 해시태그 추출 중 (Sim 2)...")
        tags2 = extract_hashtags(texts2, f"{self.target_store} 전략 후")

        blue_palette   = ["#1565C0", "#1976D2", "#2196F3", "#42A5F5", "#1A237E",
                          "#283593", "#303F9F", "#3949AB", "#0D47A1", "#1E88E5"]
        orange_palette = ["#E65100", "#F57C00", "#FB8C00", "#FF9800", "#BF360C",
                          "#D84315", "#E64A19", "#F4511E", "#FF6F00", "#FFA000"]

        fig, axes = plt.subplots(1, 2, figsize=(14, 6))
        fig.suptitle(
            f"[{self.target_store}] 핵심 키워드 해시태그 비교",
            fontsize=14, fontweight="bold"
        )
        fig.patch.set_facecolor("#FFFFFF")

        draw_hashtag_cloud(axes[0], tags1, "Sim 1 (전략 전)", blue_palette)
        draw_hashtag_cloud(axes[1], tags2, "Sim 2 (전략 후)", orange_palette)

        plt.tight_layout()
        self._savefig("01_wordcloud", fmt="png")
    ...
    def analysis_2_rating_spread(self) -> Dict:
        ...
        for df, color, label in [
            ...
            s = df["rating"].dropna() if "rating" in df.columns else pd.Series(dtype=float)
            if len(s) >= 2 and s.std() > 0:
                ...
                ax.axvline(s.iloc[0], color=color, lw=2.5, label=f"{label} (n={len(s)})")

        ax.set_xlim(1, 5)
        ax.set_xticks([1, 2, 3, 4, 5])
        ax.set_xlabel("점수", fontsize=10)
        ax.set_ylabel("밀도", fontsize=10)
        ax.set_title("종합 평점", fontsize=11, fontweight="bold")
        ...
        # ── 태그 카운트 막대 ──
        tags1 = self._get_store_tags(self.store_ratings1, self.target_store)
        tags2 = self._get_store_tags(self.store_ratings2, self.target_store)
        tag_keys = list(tags1.keys())
        x_t = np.arange(len(tag_keys))
        t_width = 0.35

        fig3, ax3 = plt.subplots(figsize=(8, 5))
        tbars1 = ax3.bar(x_t - t_width / 2, [tags1[k] for k in tag_keys],
                         t_width, label="Sim1 (전략 전)", color=COLOR_SIM1, alpha=0.85)
        tbars2 = ax3.bar(x_t + t_width / 2, [tags2[k] for k in tag_keys],
                         t_width, label="Sim2 (전략 후)", color=COLOR_SIM2, alpha=0.85)
        for bar in tbars1:
            v = bar.get_height()
            if v > 0:
                ax3.text(bar.get_x() + bar.get_width() / 2, v + 0.3,
                         str(int(v)), ha="center", va="bottom", fontsize=9, color=COLOR_SIM1)
        for bar in tbars2:
            v = bar.get_height()
            if v > 0:
                ax3.text(bar.get_x() + bar.get_width() / 2, v + 0.3,
                         str(int(v)), ha="center", va="bottom", fontsize=9, color=COLOR_SIM2)
        ax3.set_xticks(x_t)
        ax3.set_xticklabels(tag_keys, fontsize=12)
        ax3.set_ylabel("태그 선택 수", fontsize=11)
        ax3.set_title(
            f"[{self.target_store}] 맛·가성비·분위기·서비스 태그 선택 수",
            fontsize=13, fontweight="bold",
        )
        ax3.legend(fontsize=10)
        ax3.grid(axis="y", alpha=0.3)
        plt.tight_layout()
        self._savefig("02c_tag_counts")
        ...
        print(f"  [2] Rating: 평균 {result['avg_rating_before']:.2f} → {result['avg_rating_after']:.2f}"
              ...
              f" | 태그 맛{tags2['맛']}/가성비{tags2['가성비']}/분위기{tags2['분위기']}/서비스{tags2['서비스']}")
        return result
    ...
    def analysis_5_purpose_analysis(self) -> Dict:
        ...
        def purpose_avg_rating(df: pd.DataFrame) -> Dict[str, float]:
            ...
            for p in PURPOSE_KEYWORDS:
                ...
                result[p] = float(sub["rating"].mean()) if len(sub) > 0 and "rating" in sub.columns else 0.0
            return result
    ...
    def analysis_7_radar_chart(self) -> Dict:
        ...
        def compute_metrics(df: pd.DataFrame, ratings_json: Dict) -> Dict[str, Dict]:
            ...
            for store, grp in df.groupby("visited_store"):
                avg_rt = float(grp["rating"].mean()) if "rating" in grp.columns else 0.0
                ...
                sat = float((grp["rating"].dropna() >= 4).mean() * 100) if "rating" in grp.columns else 0.0

                z_gen = 0.0
                if "generation" in grp.columns:
                    z_mask = grp["generation"].str.startswith(("Z1", "Z2"))
                    z_gen  = float(z_mask.sum() / max(len(grp), 1) * 100)
        ...
        def _flatten_ratings(rj: Dict) -> Dict:
            """store_ratings.json의 stores 배열을 {store_name: store_dict} flat dict로 변환."""
            return {s["store_name"]: s for s in rj.get("stores", []) if "store_name" in s}

        metrics2 = compute_metrics(self.df2, _flatten_ratings(self.store_ratings2))
        metrics1 = compute_metrics(self.df1, _flatten_ratings(self.store_ratings1))
        ...
        radar_metrics = ["방문수", "평균평점", "재방문율", "만족도", "Z세대비율"]
        raw_data = {
            ...
            "만족도":    [metrics2.get(s, {}).get("satisfaction", 0) for s in top5],
            "Z세대비율": [metrics2.get(s, {}).get("z_gen_ratio",  0) for s in top5],
        }
    ...
    # ──────────────────────────────────────────────────────────────
    # 안전성 진단 (Safety Diagnosis)
    # ──────────────────────────────────────────────────────────────

    def analysis_safety_diagnosis(self) -> Dict:
        """11개 지표 기반 역효과 자동 탐지 및 안전성 점수 산출."""
        ms = self.metrics_summary
        findings: List[Dict] = []
        ...
        pur    = ms.get("purpose",     {})
        hourly = ms.get("hourly",      {})
        ret    = ms.get("retention",   {})
        tags_b = rt.get("tags_before", {})
        tags_a = rt.get("tags_after",  {})

        # 1. 전체 방문수
        v1, v2 = ov.get("visits_before", 0), ov.get("visits_after", 0)
        pct_v  = (v2 - v1) / max(v1, 1) * 100
        if pct_v >= 0:
            add("전체 방문 수", v1, v2, pct_v, "%", "순기능", f"방문 수 {pct_v:+.1f}% 증가")
        elif pct_v >= -10:
            add("전체 방문 수", v1, v2, pct_v, "%", "관찰 필요", f"방문 수 {pct_v:+.1f}% 소폭 감소")
        else:
            add("전체 방문 수", v1, v2, pct_v, "%", "역효과", f"방문 수 {pct_v:+.1f}% 감소 — 전략이 고객 유입에 부정적 영향")

        # 2. 평균 평점
        r1v, r2v = rt.get("avg_rating_before", 0), rt.get("avg_rating_after", 0)
        dr = r2v - r1v
        if dr >= 0:
            add("평균 평점", r1v, r2v, dr, "점", "순기능", f"평균 평점 {dr:+.2f}점 상승")
        elif dr >= -0.15:
            add("평균 평점", r1v, r2v, dr, "점", "관찰 필요", f"평균 평점 {dr:+.2f}점 소폭 하락")
        else:
            add("평균 평점", r1v, r2v, dr, "점", "역효과", f"평균 평점 {dr:+.2f}점 하락 — 고객 경험 저하 위험")

        # 3. 만족도(4점↑)
        s1, s2 = rt.get("satisfaction_rate_before", 0), rt.get("satisfaction_rate_after", 0)
        ds = s2 - s1
        if ds >= 0:
            add("만족도(4점 이상)", s1, s2, ds, "%p", "순기능", f"만족도 {ds:+.1f}%p 상승")
        elif ds >= -5:
            add("만족도(4점 이상)", s1, s2, ds, "%p", "관찰 필요", f"만족도 {ds:+.1f}%p 소폭 하락")
        else:
            add("만족도(4점 이상)", s1, s2, ds, "%p", "역효과", f"만족도 {ds:+.1f}%p 하락 — 고객 만족 저하")

        # 4. Y세대 비중
        gen_b, gen_a = gen.get("before", {}), gen.get("after", {})
        y1, y2 = gen_b.get("Y", 0), gen_a.get("Y", 0)
        dy = y2 - y1
        if dy >= 0:
            add("Y세대 방문 비중", y1, y2, dy, "%p", "순기능", f"Y세대 비중 {dy:+.1f}%p 증가")
        elif dy >= -3:
            add("Y세대 방문 비중", y1, y2, dy, "%p", "관찰 필요", f"Y세대 비중 {dy:+.1f}%p 소폭 감소")
        else:
            add("Y세대 방문 비중", y1, y2, dy, "%p", "역효과", f"Y세대 방문 비중 {dy:+.1f}%p 감소 — 핵심 소비층 이탈 우려")
        ...
        dz  = z_a - z_b
        if dz >= 0:
            add("Z세대(Z1+Z2) 유입", z_b, z_a, dz, "%p", "순기능", f"Z세대 비중 {dz:+.1f}%p 증가 — 젊은 층 유입 성공")
        else:
            add("Z세대(Z1+Z2) 유입", z_b, z_a, dz, "%p", "관찰 필요", f"Z세대 비중 {dz:+.1f}%p 감소")

        # 6. 점심 시간대 트래픽
        tr_b    = hourly.get("traffic_before", {})
        tr_a    = hourly.get("traffic_after",  {})
        lunch_b = tr_b.get(12, 0)
        lunch_a = tr_a.get(12, 0)
        if lunch_b > 0:
            dl = (lunch_a - lunch_b) / lunch_b * 100
            if dl >= 0:
                add("점심 시간대 트래픽", lunch_b, lunch_a, dl, "%", "순기능", f"점심 트래픽 {dl:+.1f}% 증가")
            elif dl >= -10:
                add("점심 시간대 트래픽", lunch_b, lunch_a, dl, "%", "관찰 필요", f"점심 트래픽 {dl:+.1f}% 소폭 감소")
            else:
                add("점심 시간대 트래픽", lunch_b, lunch_a, dl, "%", "역효과", f"점심 트래픽 {dl:+.1f}% 감소 — 점심 매출 공백 발생 위험")

        # 7. 생활베이스형 비중
        pr_b = pur.get("purpose_ratio_before", {})
        pr_a = pur.get("purpose_ratio_after",  {})
        lb1, lb2 = pr_b.get("생활베이스형", 0), pr_a.get("생활베이스형", 0)
        dlb = lb2 - lb1
        if dlb >= 0:
            add("생활베이스형 비중", lb1, lb2, dlb, "%p", "순기능", f"생활베이스형 비중 {dlb:+.1f}%p 증가 — 단골 고객 유지")
        elif dlb >= -5:
            add("생활베이스형 비중", lb1, lb2, dlb, "%p", "관찰 필요", f"생활베이스형 비중 {dlb:+.1f}%p 소폭 감소")
        else:
            add("생활베이스형 비중", lb1, lb2, dlb, "%p", "역효과", f"생활베이스형 비중 {dlb:+.1f}%p 감소 — 평일 고정 매출 약화 우려")

        # 8. 사적모임형 비중
        sp1, sp2 = pr_b.get("사적모임형", 0), pr_a.get("사적모임형", 0)
        dsp = sp2 - sp1
        if dsp >= 0:
            add("사적모임형 비중", sp1, sp2, dsp, "%p", "순기능", f"사적모임형 비중 {dsp:+.1f}%p 증가 — 모임·데이트 수요 흡수")
        else:
            add("사적모임형 비중", sp1, sp2, dsp, "%p", "관찰 필요", f"사적모임형 비중 {dsp:+.1f}%p 감소")

        # 9. 고객 순증감
        new_u   = ret.get("new_users", 0)
        churned = ret.get("churned",   0)
        net     = new_u - churned
        if net >= 0:
            add("고객 순증감", float(churned), float(new_u), float(net), "명", "순기능",
                f"신규 유입({new_u}명) > 이탈({churned}명) — 순증 {net}명")
        else:
            add("고객 순증감", float(churned), float(new_u), float(net), "명", "역효과",
                f"이탈({churned}명) > 신규 유입({new_u}명) — 고객 기반 축소")

        # 10. 가성비 태그
        gb, ga = tags_b.get("가성비", 0), tags_a.get("가성비", 0)
        dg = ga - gb
        if dg >= 0:
            add("가성비 인식", float(gb), float(ga), float(dg), "건", "순기능", f"가성비 태그 {dg:+d}건 증가")
        elif dg >= -2:
            add("가성비 인식", float(gb), float(ga), float(dg), "건", "관찰 필요",
                f"가성비 태그 {dg:+d}건 감소 — '비싸 보인다'는 인식 소폭 증가 가능")
        else:
            add("가성비 인식", float(gb), float(ga), float(dg), "건", "역효과", f"가성비 태그 {dg:+d}건 감소 — 가성비 인식 악화")

        # 11. 분위기 태그
        ab, aa = tags_b.get("분위기", 0), tags_a.get("분위기", 0)
        da = aa - ab
        if da >= 0:
            add("분위기 만족도", float(ab), float(aa), float(da), "건", "순기능", f"분위기 태그 {da:+d}건 증가 — 공간 개선 효과 반영")
        else:
            add("분위기 만족도", float(ab), float(aa), float(da), "건", "관찰 필요", f"분위기 태그 {da:+d}건 감소")

        # 위험도 점수
        weights     = {"역효과": 15, "관찰 필요": 5, "순기능": 0}
        risk_score  = min(100, sum(weights[f["verdict"]] for f in findings))
        순기능_cnt  = sum(1 for f in findings if f["verdict"] == "순기능")
        관찰_cnt    = sum(1 for f in findings if f["verdict"] == "관찰 필요")
        역효과_cnt  = sum(1 for f in findings if f["verdict"] == "역효과")
        ...
        print(f"  [안전성] 위험도: {risk_score}/100 | 순기능:{순기능_cnt} 관찰:{관찰_cnt} 역효과:{역효과_cnt}")
        return result
    ...
    def analysis_tradeoff(self) -> Dict:
        """전략 트레이드오프 — 얻은 것 vs 잃은 것."""
        ms     = self.metrics_summary
        gains:  List[Dict] = []
        losses: List[Dict] = []

        gen_b  = ms.get("generation", {}).get("before", {})
        gen_a  = ms.get("generation", {}).get("after",  {})
        pr_b   = ms.get("purpose",    {}).get("purpose_ratio_before", {})
        pr_a   = ms.get("purpose",    {}).get("purpose_ratio_after",  {})
        hourly = ms.get("hourly", {})
        tr_b   = hourly.get("traffic_before", {})
        tr_a   = hourly.get("traffic_after",  {})
        rt     = ms.get("rating", {})
        ...
        print(f"  [트레이드오프] 순기능 {len(gains)}건 / 손실 {len(losses)}건")
        return result
    ...
    def _format_safety_for_prompt(self) -> str:
        ...
        순기능 = safety.get("순기능_count", 0)
        관찰 = safety.get("관찰_count", 0)
        역효과 = safety.get("역효과_count", 0)
        lines = [
            f"- 위험도 점수: {risk_score}/100",
            f"- 순기능 {순기능}건, 관찰 필요 {관찰}건, 역효과 {역효과}건",
        ]
        for f in safety.get("findings", []):
            if f["verdict"] in ("역효과", "관찰 필요"):
                lines.append(f"- [{f['verdict']}] {f['metric']}: {f['message']}")
        return "\n".join(lines)
...
위 데이터를 바탕으로 **반드시 아래 형식 그대로** 한국어로 작성해 주세요.
숫자는 위 데이터에서 그대로 인용하세요. 섹션 헤더는 변경하지 마세요.

---

## 전략의 효과 분석

(전략 적용 후 방문 수 변화율, 평균 평점 변화, 만족도 변화를 수치와 함께 서술. 2~3문장.)

## 바뀐 주 고객층의 특성

(가장 크게 증가한 세대·방문 목적·성별을 수치와 함께 서술. 2~3문장.)

## 재방문율 및 고객 충성도

(기존 고객 유지율, 신규 유입 vs 이탈 수치 서술. 향후 충성도 강화 제언 포함. 2~3문장.)

## 역효과 분석

(안전성 진단에서 감지된 역효과·관찰 필요 항목의 원인 추론과 구체적 대응 방안. 없으면 "특이 역효과 없음"으로 작성. 2~3문장.)

## 향후 권장 사항

(데이터에서 발견된 기회·리스크 기반으로 구체적 액션 3가지를 불릿으로 작성.)"""
...
'{top_p_up}' 유형 고객의 증가세를 유지하기 위해 해당 고객층이 선호하는 분위기·메뉴·서비스 요소를 강화하는 것을 권장합니다. 또한 {top_gen_up} 세대를 주 타겟으로 한 맞춤형 마케팅 채널(SNS, 리뷰 플랫폼)을 활용하고, {top_gen_down} 세대의 이탈 원인을 세부 분석하여 개선 방안을 수립하는 것이 중요합니다."""
    ...
    def generate_all(self) -> Path:
        ...
        def _safe(name: str, fn):
            try:
                ...
                print(f"  ⚠️ [{name}] 건너뜀: {e}")
                return {}

        print("[분석 시작]")
        r1  = _safe("1_overview",            self.analysis_1_overview)
        r2  = _safe("2_rating_spread",       self.analysis_2_rating_spread)
        r3  = _safe("3_hourly_traffic",      self.analysis_3_hourly_traffic)
        r4  = _safe("4_generation_impact",   self.analysis_4_generation_impact)
        r5  = _safe("5_purpose_analysis",    self.analysis_5_purpose_analysis)
        r6  = _safe("6_retention",           self.analysis_6_retention)
        r7  = _safe("7_radar_chart",         self.analysis_7_radar_chart)
        r9  = _safe("9_agent_type",          self.analysis_9_agent_type)
        r10 = _safe("10_gender_composition", self.analysis_10_gender_composition)
        r11 = _safe("11_crosstab_heatmap",   self.analysis_11_crosstab_heatmap)

        r_safety   = _safe("safety_diagnosis", self.analysis_safety_diagnosis)
        r_tradeoff = _safe("tradeoff",         self.analysis_tradeoff)

        print("\n[LLM 종합 평가 생성 중...]")
        try:
            summary_text = self.analysis_8_llm_summary()
        except Exception as e:
            print(f"  ⚠️ [8_llm_summary] 건너뜀: {e}")
            summary_text = "(LLM 요약 생성 실패)"

        report_path = self._write_markdown_report(
            r1, r2, r3, r4, r5, r6, r7, r9, r10, r11,
            r_safety, r_tradeoff, summary_text
        )
    ...
    def _write_markdown_report(
        ...
        # 태그 카운트 테이블 행
        tags_b = r2.get("tags_before", {})
        tags_a = r2.get("tags_after",  {})
        tag_names = ["맛", "가성비", "분위기", "서비스"]
        tag_rows = "\n".join(
            f"| {t} | {tags_b.get(t, 0)} | {tags_a.get(t, 0)} | {tags_a.get(t, 0) - tags_b.get(t, 0):+d} |"
            for t in tag_names
        )

        # 안전성 진단 테이블
        findings     = r_safety.get("findings", [])
        risk_score   = r_safety.get("risk_score", 0)
        순기능_cnt   = r_safety.get("순기능_count", 0)
        관찰_cnt     = r_safety.get("관찰_count", 0)
        역효과_cnt   = r_safety.get("역효과_count", 0)
        verdict_icon = {"순기능": "✅", "관찰 필요": "🔍", "역효과": "⚠️"}
        findings_rows = "\n".join(
            f"| {verdict_icon.get(f['verdict'], '')} {f['verdict']} "
            f"| {f['metric']} | {f['before']:.1f} → {f['after']:.1f}{f['unit']} "
            f"| {f['delta']:+.1f}{f['unit']} | {f['message']} |"
            for f in findings
        )

        # 트레이드오프 테이블
        gains  = r_tradeoff.get("gains",  [])
        losses = r_tradeoff.get("losses", [])
        max_len = max(len(gains), len(losses), 1)
        tradeoff_rows = "\n".join(
            f"| {gains[i]['metric']} {gains[i]['delta']} "
            f"| ⇄ | {losses[i]['metric']} {losses[i]['delta']} |"
            if i < len(gains) and i < len(losses)
            else (
                f"| {gains[i]['metric']} {gains[i]['delta']} | ⇄ | — |"
                if i < len(gains)
                else f"| — | ⇄ | {losses[i]['metric']} {losses[i]['delta']} |"
            )
...
> **위험도 점수:** {risk_score}/100 | ✅ 순기능: {순기능_cnt}건 | 🔍 관찰 필요: {관찰_cnt}건 | ⚠️ 역효과: {역효과_cnt}건
...
![rating_spread](figures/02_rating_spread.svg)

![satisfaction_rate](figures/02b_satisfaction_rate.svg)

### 맛·가성비·분위기·서비스 태그 선택 수

| 태그 | Sim 1 (전략 전) | Sim 2 (전략 후) | 변화 |
|:---:|:--------------:|:--------------:|:----:|
{tag_rows}

![tag_counts](figures/02c_tag_counts.svg)
...
![hourly_traffic](figures/03_hourly_traffic.svg)
...
![generation_impact](figures/04_generation_impact.svg)
...
![purpose_analysis](figures/05_purpose_analysis.svg)
...
![retention](figures/06_retention.svg)
...
> **비교 지표:** 방문수, 평균평점, 재방문율, 만족도, Z세대비율 (정규화 0~100)
...
| 만족도(4점↑) | {tm1.get('satisfaction', 0):.1f}% | {tm2.get('satisfaction', 0):.1f}% |
| Z세대 비율 | {tm1.get('z_gen_ratio', 0):.1f}% | {tm2.get('z_gen_ratio', 0):.1f}% |

![radar_chart](figures/07_radar_chart.svg)
...
![agent_type](figures/09_agent_type.svg)
...
![gender_composition](figures/10_gender_composition.svg)
...
![crosstab_heatmap](figures/11_crosstab_heatmap.svg)
