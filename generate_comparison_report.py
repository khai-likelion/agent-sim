...
ComparisonReportGenerator 클래스 기반 8가지 핵심 지표 분석:
...
GENERATION_ORDER = ["Z1", "Z2", "Y", "X", "S"]
GEN_COLORS       = ["#4E79A7", "#F28E2B", "#E15759", "#76B7B2", "#59A14F"]
...
class ComparisonReportGenerator:
    ...
    def _load_sim_data(
        ...
        _EMPTY_COLS = [
            ...
            "taste_rating", "value_rating", "atmosphere_rating", "reason",
        ]
    ...
    def _plot_wordcloud(self, t1: pd.DataFrame, t2: pd.DataFrame):
        """reason + visited_category 텍스트로 워드클라우드 생성."""
        try:
    ...
    def analysis_7_radar_chart(self) -> Dict:
        ...
        # ── 레이더 차트 ──
        N      = len(radar_metrics)
        angles = [n / float(N) * 2 * np.pi for n in range(N)] + [0]   # 닫기
...
위 데이터를 바탕으로 **3~5 문단 한국어**로 아래 세 가지를 분석해 주세요:
1. **전략의 효과**: 방문·만족도 변화와 그 원인
2. **바뀐 주 고객층의 특성**: 증가한 세대·방문 목적과 시사점
3. **향후 권장 사항**: 데이터에서 발견된 기회 요소와 리스크"""
...
세대 분포에서 {top_gen_up} 세대의 비율이 {gen_deltas[top_gen_up]:+.1f}%p로 가장 크게 증가하였고, {top_gen_down} 세대는 {gen_deltas[top_gen_down]:+.1f}%p 감소하였습니다. 방문 목적 유형에서는 '{top_p_up}'이 {p_deltas[top_p_up]:+.1f}%p 증가하여 핵심 고객층으로 부상한 반면, '{top_p_down}'은 {p_deltas[top_p_down]:+.1f}%p 감소하였습니다. 이는 전략이 특정 라이프스타일 그룹에 더 효과적으로 작용했음을 의미합니다.
...
'{top_p_up}' 유형 고객의 증가세를 유지하기 위해 해당 고객층이 선호하는 분위기·메뉴·서비스 요소를 강화하는 것을 권장합니다. 또한 {top_gen_up} 세대를 주 타겟으로 한 맞춤형 마케팅 채널(SNS, 리뷰 플랫폼)을 활용하고, {top_gen_down} 세대의 이탈 원인을 세부 분석하여 개선 방안을 수립하는 것이 중요합니다."""
    ...
    def generate_all(self) -> Path:
        ...
        report_path = self._write_markdown_report(r1, r2, r3, r4, r5, r6, r7, summary_text)
...
*Report generated by `ComparisonReportGenerator` — ABM Simulation Analysis*
*{now}*
"""
