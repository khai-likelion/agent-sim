...
def load_environment_from_dir(store_json_dir: Path, target_store: str = None):
    ...
    GlobalStore.reset_instance() ## 이게 없으면 시뮬레이션 1의 평점이 시뮬레이션 2로 오염됨
...
def compare_results(before_visit_df, after_visit_df, target_store: str):
    ...
    for label, df in [("전략 전", before_visit_df), ("전략 후", after_visit_df)]:
        ...
        avg_rating = target_visits["rating"].mean() if len(target_visits) > 0 else 0
...
REPORT_DIR = PROJECT_ROOT / "data" / "raw" / "전략 md 파일들"

def _find_report_path(target_store: str) -> Path:
    """
    target_store 이름으로 X-Report 파일을 자동 탐색.

    탐색 순서:
    1. {target_store}_report.md  (공백 포함 그대로)
    2. {target_store}_report.md  (공백 제거)
    3. 폴더 내 *_report.md 파일 중 target_store를 포함하는 파일
    """
    candidates = [
        REPORT_DIR / f"{target_store}_report.md",
        REPORT_DIR / f"{target_store.replace(' ', '')}_report.md",
    ]
    for path in candidates:
        if path.exists():
            return path

    # fallback: 폴더 내 파일명에 target_store(공백 제거)가 포함된 파일
    normalized = target_store.replace(" ", "")
    for path in REPORT_DIR.glob("*_report.md"):
...
async def main():
    ...
    parser.add_argument("--report", type=str, default=None,
                        help="X-Report 경로 (미지정 시 data/raw/전략 md 파일들/ 에서 자동 탐색)")
    ...
    if args.report is None:
        found = _find_report_path(args.target_store)
        if found:
            args.report = str(found)
        else:
            print(f"[오류] '{args.target_store}' X-Report를 찾을 수 없습니다: {REPORT_DIR}")
            return
    ...
    # 백업 확인 (없으면 생성)
    ## .bak 파일이 없을 때만 원본을 백업. 이미 있으면 스킵 -> 여러 번 실행해도 원본 덮어씌워지지 않음
