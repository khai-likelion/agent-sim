...
class ActionAlgorithm:
    ...
    def step4_evaluate_and_feedback(
        ...
        if result and all(key in result for key in ["taste_rating", "value_rating", "atmosphere_rating"]):
            # 점수 범위 검증 (1~5)
            taste = max(1, min(5, int(result["taste_rating"])))
            value = max(1, min(5, int(result["value_rating"])))
