...
## 같은 구역에 사는 에이전트들이 정확히 같은 좌표에 몰리지 않도록 최대 150m 반경으로 흩뿌릴 때 쓰는 상수.
...
class VisitRecord:
    ...
    visit_datetime: str  # 방문 시각
    store_name: str      # 방문한 매장명
    category: str        # 음식 카테고리
    rating: int          # 별점 (1~5)
    comment: str = ""    # 자연어 코멘트
    selected_tags: List[str] = field(default_factory=list)  # 선택한 태그
...
class GenerativeAgent:
    ...
    """
    ## 식별/페르소나 필드
    id: int                          # 시뮬레이션 내 순번 (1~160)
    persona_id: str                  # "P001"~"P113" (유동), "R001"~"R047" (상주)
    group_type: str                  # 생활베이스형 / 사적모임형 / 공적모임형 / 가족모임형
    group_size: int                  # 1인, 2인, 4인
    generation: str                  # Z1, Z2, Y, X, S (세대)
    gender_composition: str          # 남, 여, 혼성
    agent_type: str                  # "유동" or "상주"
    natural_language_persona: str = ""    # personas_160.md 본문 전체 (LLM 프롬프트에 직접 주입)
    housing_type: Optional[str] = None      # 상주만: "다세대(빌라)", "단독·연립(주택)" 등

    ## 위치 필드
    recent_history: List[VisitRecord] = field(default_factory=list) # 최대 15개 방문 기록(메모리)
    current_location: Optional[Any] = field(default=None, repr=False) # 현재 위치 (OSMnx 노드 등)
    home_location: Optional[Tuple[float, float]] = field(default=None, repr=False) # 집(상주 전용, 유동은 None)

    # 유동 에이전트 전용: 진입 지점 (home_location과 분리)
    entry_point: Optional[Tuple[float, float]] = field(default=None, repr=False) # 진입(유동 전용)
    entry_time_slot: Optional[str] = field(default=None, repr=False)  # 진입 시간대: "아침", "점심", "저녁", "야식"
    left_mangwon: bool = field(default=False, repr=False)  # 유동 에이전트가 망원동 떠남 여부

    ## is_resident, is_floating은 action_algorithm.py에서 분기 처리에 사용.
    @property
    ...
    ## 방문할 때마다 action_algorithm.py에서 호출. 최대 15개만 유지 (메모리 슬라이딩 윈도우)
    def add_visit(self, store_name: str, category: str, rating: int, visit_datetime: str = "", comment: str = "", selected_tags: Optional[List[str]] = None):
        record = VisitRecord(
        ...
        if len(self.recent_history) > 15:
            self.recent_history = self.recent_history[-15:] ## 15개 초과 시 오래된 것 제거
    ...
    def get_memory_context(self, current_date: str = "") -> str:
        ...
        if not self.recent_history:
            ...
            for v in self.recent_history[-5:]:
                line = f"  - {v.store_name} ({v.category}): {rating_text.get(v.rating, '?')}"
                if v.comment:
                    line += f' → "{v.comment}"'
                lines.append(line)
    ...
    ## __main__(수동 테스트용) 블록에서만 사용, 실제 시뮬에서 미사용
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "home_location": list(self.home_location) if self.home_location else None,
