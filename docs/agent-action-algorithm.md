...
### 2-4. 하이브리드 모델 라우팅 (단계별 모델 분리)

비용·품질 균형을 위해 단계별로 다른 Gemini 모델을 사용한다.

| 단계 | 모델 | 환경변수 | 이유 |
|------|------|---------|------|
| Step 2 (업종 선택) | `gemini-2.5-flash-lite` | `STEP_LITE_MODEL` | 간단한 카테고리 선택 |
| Step 3 (매장 선택) | `gemini-2.5-flash-lite` | `STEP_LITE_MODEL` | 목록 중 선택 |
| Step 4 (평가·피드백) | `gemini-2.5-flash` | `STEP4_MODEL` | 페르소나 기반 리뷰 생성 |
| Step 5 (다음 행동) | `gemini-2.5-flash-lite` | `STEP_LITE_MODEL` | 행동 선택지 중 선택 |

```python
# ActionAlgorithm.__init__ 내 env 읽기
...
# 각 Step 호출
response = await self._call_llm_async(prompt, model=self.step_lite_model)  # Step2/3/5
response = await self._call_llm_async(prompt, model=self.step4_model)      # Step4
```

> `.env`에서 `STEP_LITE_MODEL` / `STEP4_MODEL` 값을 바꿔 전체를 모델 교체 없이 전환 가능.
...
[매장명]
  ...
  설명: 망원동 골목 안 가정식 백반집... (최대 250자)
  에이전트 평점: 4.2/5 (8건), 태그: 맛 5, 가성비 3, 분위기 2, 서비스 1
    - R001: 좋음 ['맛', '가성비']
    - P012: 매우좋음 ['맛']
```

**후보 선택:** `search_ranked_stores()` — Softmax(T=0.5) 가중 샘플링 20개 + 계층화(상위60%·중위20%·하위20%).
...
| 구분 | 내용 |
|------|------|
| 방식 | LLM (페르소나 말투 + 페르소나 기반 평가 관점 자율 선택) |
...
**평가 관점 (LLM 자율 선택):** 페르소나(세대·방문 목적·동행 구성)에 맞는 하나를 LLM이 스스로 선택 — `맛/퀄리티` / `가성비` / `매장 분위기·인테리어` / `직원 서비스·친절도` / `매장 청결·위생` / `특색있는 메뉴`

**selected_tags 선택 기준:** 맛/가성비/분위기/서비스 각각 기준 충족 시에만 선택. 없으면 빈 배열 가능.
