...
2. 평가 관점: 이번 리뷰는 철저히 "맛/퀄리티" 관점에서만 평가하세요.
...
2. 주요 장점 (selected_tags): "맛", "가성비", "분위기", "서비스" 중 0~2개 선택

반드시 아래 JSON 형식으로만 응답하세요:
{"rating": 1~5, "selected_tags": ["맛", "가성비"], "comment": "한줄 평가"}
```
...
| `{focus_aspect}` | `맛/퀄리티` (랜덤) | `random.choice(focus_aspects)` |
