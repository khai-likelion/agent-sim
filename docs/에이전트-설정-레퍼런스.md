...
리뷰 멘트 다양성: tone_instruction(세대별 말투) + focus_aspect(랜덤 관점) + comment 규칙 + rating/selected_tags 개편
...
`action_algorithm.py` (Step 4) — agent.generation별 tone_instruction, random.choice(focus_aspects)
