...
`agent-settings-reference.md`를 기반으로, **실제 반영된 코드**와 **설명**을 토글로 펼쳐서 볼 수 있는 문서입니다.

> GitHub, VS Code, Notion 등에서 `▶` 를 클릭하면 코드·설명이 펼쳐집니다.
...
<details>
<summary><strong>▶ tone_instruction (세대별 말투) + focus_aspect (평가 관점 랜덤)</strong></summary>
...
<details>
<summary><strong>▶ search_ranked_stores — Softmax T=0.5 + 60/25/15 계층화</strong></summary>

### 설정 이유
exploit 10 + explore 5 고정 → 롱테일 매장 0.7% 진입.  
Softmax 확률 비례 추출 + 상위60%/중위25%/하위15%로 롱테일도 LLM 후보에 포함.
...
| 문서 | 설명 |
|------|------|
| [agent-settings-reference.md](./agent-settings-reference.md) | 설정·이유 한눈에 요약 |
| [agent-decision-flow.md](./agent-decision-flow.md) | Step 1~5 데이터 흐름 |
| [agent-upgrade-summary.md](./agent-upgrade-summary.md) | 고도화 상세 (문제·원인·해결) |
